<div align="center">
<img src="https://github.com/CHAT-DISPARU/CHAT-DISPARU/blob/main/banner.gif">
</div>

---

<h1 align="center">About Me 😁​</h1>

```c
#include <stdlib.h>

typedef struct  s_gajanvie
{
    char        *name;
    int         age;
    char        *school;
    char        *best_project_made;
    char        **hobbies;
}               t_gajanvie;

int main(void)
{
    t_gajanvie  me;
    char        *my_hobbies[] = {
        "Learning new things about coding, math, and physics",
        "Sports in general, especially Muay Thai and trail running",
        NULL
    };

    me.name = "Gaetan Franca";
    me.age = 17;
    me.school = "42 Angouleme";
    me.best_project_made = "MiniRT";
    me.hobbies = my_hobbies;
    return (0);
}
```
---

<br/>
<p align="center">
    <img src="https://streak-stats.demolab.com?user=CHAT-DISPARU&theme=tokyonight&hide_border=true" alt="GitHub Streak" width="40%" />
    <img src="https://github-readme-stats.vercel.app/api/top-langs?username=TFHD&locale=en&hide_title=false&layout=compact&card_width=320&theme=radical&langs_count=5&bg_color=30,090a0c,303b51&title_color=fff&text_color=fff&hide_border=false&order=2" height="150" alt="languages graph"  />
</p>

<div align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=CHAT-DISPARU&theme=tokyo-night&bg_color=0d1117&color=0e75b6&line=0e75b6&point=ffffff&hide_border=true" alt="Activity Graph" />
</div>
